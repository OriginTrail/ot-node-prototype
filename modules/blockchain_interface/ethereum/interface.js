...
module.exports = function (){
	...
	var interface = {
		getFingerprintByBatchHash: function(address, batch_id_hash){
			...
			return contract_instance.getFingerprintByBatchHash(address,batch_id_hash, {from: web3.eth.accounts[0]});
		},
		addFingerprint: function(batch_id,batch_id_hash, graph_hash){
            console.log('id : ' + batch_id);
            console.log('id_hash : ' + batch_id_hash);
            console.log('graph_hash : ' + graph_hash);
            web3.personal.unlockAccount(web3.eth.accounts[0], config.blockchain.settings.ethereum.password, 10);
			contract_instance.addFingerPrint(batch_id,batch_id_hash, graph_hash, {from: web3.eth.accounts[0]});
			return true;
		},
		...
		getFingerprint: function(data_holder_address, batch_id_hash, callback){
			var graph_hash = null;
            web3.personal.unlockAccount(web3.eth.accounts[0], config.blockchain.settings.ethereum.password, 10);
			graph_hash = contract_instance.getFingerprintByBatchHash(data_holder_address,batch_id_hash, {from: web3.eth.accounts[0]});

			utilities.execute_callback(callback,graph_hash);
			return graph_hash;
		}
	};
